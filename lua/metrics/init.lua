...
local function check_interval(force)
    for branch_name, branch_info in pairs(branch_map) do
        ...
        if time_diff > branch_idol_time or force then
            ...
            local start_time_utc = os.date("!%Y-%m-%d %H:%M:%S",
                                           branch_info.start_time)
            local end_time_utc = os.date("!%Y-%m-%d %H:%M:%S",
                                         branch_info.end_time)

            local query = [[
                INSERT INTO time_tracking (branch, start_time, end_time)
                VALUES (']] .. branch_name .. [[', ']] .. start_time_utc ..
                              [[', ']] .. end_time_utc .. [[')
            ]]
...
function M.start_time_tracking()
    ...
    vim.api.nvim_create_autocmd("VimLeave", {
        group = vim.api.nvim_create_augroup("MetricsTimeTrackingVimLeave",
                                            {clear = true}),
        callback = function()
...
function M.init_db()
    ...
    Job:new({
        ...
        on_stdout = function(_j, data) print("on_stdout", data) end,
        on_stderr = function(_j, data) print("on_stderr", data) end
    }):start()
end
...
function M.get_time_worked_for_current_branch_async(callback)
    ...
    Job:new({
        ...
        on_exit = function(j, _return_val)
            ...
            if #result == 0 then callback(nil) end
...
function M.print_branch_map() print(vim.inspect(branch_map)) end
